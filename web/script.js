...
const CUBF_LABELS = {
  mamh_strict:      'Filtre CUBF strict — 1000, 1010, 1211, 5000–5999',
  mamh_optional:    'Filtre CUBF élargi — 1000, 1010, 1211, 1543, 1549, 5000–5999, 8100–8199',
  mamh_plus_others: 'Filtre CUBF complet — 1000–1999, 5000–5999, 8100–8199',
};
...
let currentCubf    = 'mamh_strict';

// ─── LOAD DATA ────────────────────────────────────────────────────────────────
async function loadAll() {
  // Base files (no suffix) — used for mrc_list and fallback
  const baseFiles = ['mrc_list'];
  // Files that have CUBF-variant suffixes
  const variantBase = [
  ...
  // Deduplicated variant filenames
  const variantFiles = [
    'logements_types_mrc','logements_types_mun',
    'age_mrc','age_mun',
    'periode_mrc',
  ...
  // Files that exist only once (no suffix)
  const singleFiles = ['valeur_mrc','valeur_mun','mrc_list'];

  const toLoad = [];
  // Single files
  singleFiles.forEach(f => toLoad.push({key: f, url: `./data/${f}.json`}));
  // Variant files for each suffix
  variantFiles.forEach(f => {
    suffixes.forEach(s => {
      toLoad.push({key: `${f}_${s}`, url: `./data/${f}_${s}.json`});
    });
  });

  await Promise.all(toLoad.map(async ({key, url}) => {
    try {
...
// Helper: get data key for current CUBF mode
function dk(base) {
  const singleFiles = ['valeur_mrc','valeur_mun','mrc_list'];
  if (singleFiles.includes(base)) return base;
  return `${base}_${currentCubf}`;
}

function getAvailableYears() {
  const years = new Set();
  [
    DATA[dk('logements_types_mrc')],
    DATA[dk('logements_types_mun')],
    DATA.valeur_mrc,
    DATA.valeur_mun,
    DATA[dk('age_mrc')],
    DATA[dk('age_mun')],
...
function drawMap() {
  ...
  const anneeData = (DATA[dk('logements_types_mrc')] || []).filter(d => d.Annee == currentYear);
  const totals = {};
...
function renderLogements(content, mrcLabel, annee, filterMRC, filterYear, filterYearMRC, filterYearRangeMRC) {
  ...
  const TYPE_COLS = [
    ...
    "Habitation en commun","Logements dans un immeuble comportant deux logements ou plus",
