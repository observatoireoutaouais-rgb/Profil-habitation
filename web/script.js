...
const FOOTNOTE = `Les catégories MAMH sont d'abord identifiées selon l'univers CUBF retenu, puis classées selon le lien physique et le nombre de logements. Dans le mode « MAMH + autres catégories », les catégories détaillées restantes sont attribuées à partir du CUBF pour les immeubles hors univers MAMH.`;

const CATEGORY_MODES = ['mamh_strict','mamh_optional','mamh_plus_others'];
const MODE_BASE_FILES = ['logements_types_mrc','logements_types_mun','valeur_mrc','valeur_mun','age_mrc','age_mun','periode_mrc','periode_mun','superficie_mrc','superficie_mun'];
...
async function loadAll() {
  const staticFiles = ['mrc_list','nouveaux_logements_mrc','nouveaux_logements_mun','densite_pu_mrc','densite_pu_mun','types_nouveaux_mrc','types_nouveaux_mun'];
  const files = [...staticFiles];
  MODE_BASE_FILES.forEach(base => CATEGORY_MODES.forEach(mode => files.push(`${base}_${mode}`)));
...
function drawMap() {
  ...
  const anneeData = (getData('logements_types_mrc')).filter(d => d.Annee == currentYear);
  const totals = {};
...
function renderValeur(content, mrcLabel, annee, filterYearMRC, filterYearRangeMRC) {
  ...
  // Multi-year
  const allVals = (getData('valeur_mrc')).filter(d => d.CDNAME === (currentMRC||d.CDNAME) && d.Types === "Total des unités d'évaluation résidentielles");
  const byYear = {};
  ...
  const allData = filterYearRangeMRC(getData('valeur_mrc'));
  const byYearType = {};
