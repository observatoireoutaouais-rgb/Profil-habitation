...
def _parse_geojson_year(json_path,annee,pf_lookup,region_map):
    """GeoJSON streaming: on lit feature par feature pour ne pas charger tout en RAM."""
    try:
        import ijson
    except ImportError:
        raise RuntimeError(
            "Le repli GeoJSON nécessite le paquet 'ijson'. "
            "Installez-le avec :  pip install ijson"
        )
    rows=[]; found_codes=set()
