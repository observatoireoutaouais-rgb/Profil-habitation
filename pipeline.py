...
def _read_dbf_layout(f):
    ...
    Les noms de champs sont normalisés en minuscules (2012-2014 utilisent des majuscules)."""
    header = f.read(32)
    ...
    for annee, zip_path in sorted(SHP_ZIPS.items()):
        ...
        try:
            with zipfile.ZipFile(zip_path, "r") as zf:
                ...
                # Exclure les fichiers ADR_ (adresses) qui partagent le même suffixe
                def is_main_dbf(name):
                    u = name.upper()
                    if 'ADR_UNITE_EVALN' in u: return False
                    return (u.endswith('B05EX1_B05V_UNITE_EVALN.DBF')
                            or u.endswith('B05V_UNITE_EVALN.DBF')
                            or u.endswith('UNITE_EVALN.DBF'))
                dbf_entry = next(
                    (n for n in zf.namelist() if is_main_dbf(n)),
                    None,
                )
                if not dbf_entry:
