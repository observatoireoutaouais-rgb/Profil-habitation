...
Sources selon l'année :
  • 2012 à 2022   : SHP provincial local (ROLE{YYYY}_SHP.zip)
  • 2023 et plus  : CSV/GeoJSON provincial téléchargé depuis MAMH
                    (role_unite_p_{YYYY}.csv, repli .json)
                    Mêmes champs RL que le DBF, donc même méthode logique
                    qu'avec le SHP : filtrage par code_mun via pf_lookup.
...
def build_role_df_shp(pf_lookup,match_df):
    ...
    return all_rows,errors
# ─────────────────────────────────────────────────────────────────────────────
# Source provinciale CSV/GeoJSON pour 2023+ (équivalent du SHP des années ≤2022)
# MAMH ne publie plus de SHP après 2022; on utilise les fichiers consolidés
# role_unite_p_{YYYY}.csv (préféré) ou role_unite_p_{YYYY}.json (GeoJSON, repli).
# Mêmes champs RL que le DBF, donc même logique de filtrage par code_mun.
# ─────────────────────────────────────────────────────────────────────────────
CSV_URL  = "https://donneesouvertes.affmunqc.net/role/role_unite_p_{annee}.csv"
JSON_URL = "https://donneesouvertes.affmunqc.net/role/role_unite_p_{annee}.json"
CSV_FIELDS = ["code_mun","rl0105a","rl0302a","rl0307a","rl0308a",
              "rl0309a","rl0311a","rl0402a","rl0403a","rl0404a"]
def _download_to_file(url,dest,timeout=600):
    headers={"User-Agent":"Mozilla/5.0"}
    with requests.get(url,headers=headers,stream=True,timeout=timeout) as r:
        if r.status_code in (404,403): return False
        r.raise_for_status()
        total=int(r.headers.get("content-length",0))
        got=0; chunk_size=1<<20
        with open(dest,"wb") as f:
            for chunk in r.iter_content(chunk_size=chunk_size):
                if not chunk: continue
                f.write(chunk); got+=len(chunk)
                if total: print(f"\r    {got/1e6:.0f}/{total/1e6:.0f} MB",end="",flush=True)
                else:     print(f"\r    {got/1e6:.0f} MB",end="",flush=True)
        print()
    return True
def _parse_csv_year(csv_path,annee,pf_lookup,region_map):
    rows=[]; found_codes=set()
    # Lecture par chunks: le fichier provincial peut faire plusieurs Go.
    use=set(CSV_FIELDS)
    reader=pd.read_csv(csv_path,dtype=str,low_memory=True,
                       usecols=lambda c: c in use or c.lower() in use,
                       chunksize=200_000)
    for chunk in reader:
        chunk.columns=[c.lower() for c in chunk.columns]
        if "code_mun" not in chunk.columns: continue
        chunk["code_mun"]=chunk["code_mun"].astype(str).str.zfill(5)
        chunk=chunk[chunk["code_mun"].isin(pf_lookup)]
        if chunk.empty: continue
        for _,r in chunk.iterrows():
            code=r["code_mun"]
            nom_mun,cdname=pf_lookup[code]; found_codes.add(code)
            rows.append({"Annee":annee,"CSDNAME":nom_mun,"CDNAME":cdname,
                "Region":region_map.get(cdname),
                "rl0105a":r.get("rl0105a"),"rl0302a":r.get("rl0302a"),
                "rl0307a":r.get("rl0307a"),"rl0308a":r.get("rl0308a"),
                "rl0309a":r.get("rl0309a"),"rl0311a":r.get("rl0311a"),
                "rl0402a":r.get("rl0402a"),"rl0403a":r.get("rl0403a"),
                "rl0404a":r.get("rl0404a")})
    return rows,found_codes
def _parse_geojson_year(json_path,annee,pf_lookup,region_map):
    """GeoJSON parser using stdlib json (no ijson dependency).
    The file can be ~1 GB; fits comfortably in GitHub Actions RAM (~7 GB)."""
    import json as _json
    rows=[]; found_codes=set()
    with open(json_path,"r",encoding="utf-8") as f:
        gj=_json.load(f)
    feats=gj.get("features",[]) if isinstance(gj,dict) else []
    print(f"    {len(feats):,} features chargées en mémoire")
    for feat in feats:
        p=(feat.get("properties") or {}) if isinstance(feat,dict) else {}
        ...
        rows.append({"Annee":annee,"CSDNAME":nom_mun,"CDNAME":cdname,
            "Region":region_map.get(cdname),
            "rl0105a":s(p.get("rl0105a")),"rl0302a":s(p.get("rl0302a")),
            "rl0307a":s(p.get("rl0307a")),"rl0308a":s(p.get("rl0308a")),
            "rl0309a":s(p.get("rl0309a")),"rl0311a":s(p.get("rl0311a")),
            "rl0402a":s(p.get("rl0402a")),"rl0403a":s(p.get("rl0403a")),
            "rl0404a":s(p.get("rl0404a"))})
    # Libérer la mémoire avant de retourner (le GC s'en charge, mais on aide)
    del feats, gj
    return rows,found_codes
def build_role_df_csv(pf_lookup,match_df):
    """Source provinciale 2023+ via CSV (puis GeoJSON en repli)."""
    if not pf_lookup: return [],[]
    region_map=match_df.drop_duplicates("CDNAME").set_index("CDNAME")["Region"].to_dict()
    cache_dir=Path("./cache_provincial"); cache_dir.mkdir(exist_ok=True)
    annees_cibles=[a for a in ANNEES if a>=2023 and a not in SHP_ZIPS]
    all_rows,errors=[],[]
    for annee in annees_cibles:
        print(f"\n══ Année {annee} (CSV/GeoJSON provincial) ══")
        # 1) Tenter le CSV
        csv_path=cache_dir/f"role_unite_p_{annee}.csv"
        json_path=cache_dir/f"role_unite_p_{annee}.json"
        rows=None; found_codes=set()
        try:
            if not csv_path.exists():
                print(f"  Téléchargement CSV {CSV_URL.format(annee=annee)}")
                if _download_to_file(CSV_URL.format(annee=annee),csv_path):
                    print(f"  ✓ CSV en cache : {csv_path}")
                else:
                    csv_path.unlink(missing_ok=True)
                    print(f"  CSV indisponible pour {annee}")
            if csv_path.exists():
                print(f"  Lecture CSV…")
                rows,found_codes=_parse_csv_year(csv_path,annee,pf_lookup,region_map)
        except Exception as e:
            print(f"  ⚠  Échec CSV {annee}: {e}")
            csv_path.unlink(missing_ok=True)
            rows=None
        # 2) Repli GeoJSON
        if not rows:
            try:
                if not json_path.exists():
                    print(f"  Téléchargement GeoJSON {JSON_URL.format(annee=annee)}")
                    if not _download_to_file(JSON_URL.format(annee=annee),json_path):
                        json_path.unlink(missing_ok=True)
                        print(f"  GeoJSON indisponible pour {annee}")
                        errors.append({"annee":annee,"mun":"CSV/GeoJSON","erreur":"aucune source provinciale"})
                        continue
                    print(f"  ✓ GeoJSON en cache : {json_path}")
                print(f"  Lecture GeoJSON (streaming)…")
                rows,found_codes=_parse_geojson_year(json_path,annee,pf_lookup,region_map)
            except Exception as e:
                print(f"  ERR GeoJSON {annee}: {e}")
                errors.append({"annee":annee,"mun":"CSV/GeoJSON","erreur":str(e)})
                continue
        if not rows:
            print(f"  ⚠  Aucune ligne extraite pour {annee}.")
            continue
        # Garde-fou (même esprit que le SHP : on rejette un fichier visiblement tronqué).
        if len(rows)<1000:
            print(f"  ⚠  Seulement {len(rows):,} UE retenues – source suspecte, année ignorée.")
            continue
        all_rows.extend(rows)
        print(f"  ✓ {len(rows):,} UE retenues ({len(found_codes)} municipalités)")
    return all_rows,errors
...
def main():
    ...
    # 2012-2022 : SHP zips locaux (DBF parsing)
    rows_shp,_=build_role_df_shp(pf_lookup,MATCH)
    # 2023+ : CSV/GeoJSON provincial (téléchargé depuis MAMH)
    rows_csv,_=build_role_df_csv(pf_lookup,MATCH)
    all_rows=rows_shp+rows_csv
    ...
    # Le dashboard lit les fichiers sans suffixe ; on les fait pointer sur le mode "strict"
    # (cohérent avec le bouton "Strict" sélectionné par défaut dans l'UI).
    import shutil
    aliases=[
    ...
    print("\nAlias 'mamh_strict' → fichiers sans suffixe (lus par le dashboard) :")
    for base in aliases:
        src=DATA_DIR/f"{base}_mamh_strict.json"
        dst=DATA_DIR/f"{base}.json"
        if src.exists():
            shutil.copyfile(src,dst)
            print(f"  ✓ {dst.name}")
        else:
            print(f"  ⚠  {src.name} introuvable, alias ignoré")
